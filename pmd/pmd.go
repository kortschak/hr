...
// NewListenser returns a new Listener for the provided Bluetooth device.
...
func (l *Listener) Settings(ctx context.Context, r RecordingType, m MeasureType) ([]Setting, error) {
	return querySettings(ctx, l.cpDevice, MeasureSettings, r, m)
}
...
// Handler defines a PMD handler.
type Handler interface {
	// Handle returns the command, recording and measurement
	// types and the settings to configure notifications with.
	// The function is called with the data for all notifications
	// of the specified measurement type.
	Handle() (Command, RecordingType, MeasureType, []Setting, func(buf []byte))
}
